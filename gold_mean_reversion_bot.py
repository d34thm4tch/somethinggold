...
# NOTE: these are in raw price points (e.g. dollars per ounce for gold),
# not "pips". Gold often moves $10-30+ in a day, so $5/$8 is intentionally
# tight for a quick demo test - widen these once you've checked gold's
# recent ATR (Average True Range) on a 15-min chart, or stops will trigger
# on normal noise rather than genuine reversals.
...
def get_recent_prices(num_bars=100):
    resp = requests.get(
...
def run_once():
    ...
    session.authenticate()
    try:
        closes = get_recent_prices()
        rsi = calculate_rsi(closes)
        current_price = closes[-1]
        position = get_open_position()

        print(f"[{datetime.now()}] Price={current_price:.2f}  RSI={rsi}")
